...
export async function rewriteContentWithSEO(
  ...
  try {
    ...
    if (!geminiApiKey) {
      throw new Error("Chave de API do Gemini não configurada. Configure nas configurações da aplicação.");
    }
...
Responda APENAS com um JSON válido seguindo os 12 critérios do Google:
{
  "rewrittenContent": "conteúdo reescrito completo formatado em HTML com estrutura otimizada${keywordLink ? ` incluindo link na palavra-chave principal para ${keywordLink}` : ''}",
  ...
  "authorBio": "Bio profissional do ${authorName}${authorDescription ? ` baseada em: ${authorDescription}` : ''}"` : ''}${companyName ? `,
  "ctaSection": {
    ...
    if (error instanceof Error) {
      if (error.message.includes("API key")) {
        throw new Error("Chave da API Gemini não configurada corretamente");
      }
      if (error.message.includes("quota")) {
        throw new Error("Limite da API Gemini excedido. Tente novamente mais tarde");
      }
